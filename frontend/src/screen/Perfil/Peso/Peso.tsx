...
const Pesos: React.FC<Props> = ({ navigation }) => {
  ...
  const userId = user?.id;
    ...
    const fetchPeso = async () => {
      ...
      try {
        const response = await axios.get(`http://192.168.18.46:3000/peso/${userId}`);
  ...
  try {
  const response = await axios.put(`http://192.168.18.46:3000/user/atualizarpeso/${user?.id}`, dataAtualizada);
  ...
  const handleAddPeso = async () => {
    ...
    try {
      await axios.post("http://192.168.18.46:3000/peso", {
        user: userId,
