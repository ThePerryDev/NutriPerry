...
const GraficoConsumoAgua: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchWeeklyData = async () => {
    try {
      const response = await fetch(`http://10.68.55.153:3000/consumo-agua/weekly/${userId}`);
  ...
  const fetchMonthlyData = async () => {
    try {
      const response = await fetch(`http://10.68.55.153:3000/consumo-agua/monthly/${userId}`);
      const data = await response.json();
  ...
  const fetchYearlyData = async () => {
    try {
      const response = await fetch(`http://10.68.55.153:3000/consumo-agua/yearly/${userId}`);
      const data = await response.json();
