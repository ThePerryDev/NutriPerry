...
const SeusExercicios: React.FC<Props> = ({ navigation }) => {// Estado para rastrear o checkbox de cada refeição
  ...
  const fetchExercicios = async () => {
    try {
      ...
      const responseExercicios = await axios.get(`http://192.168.0.107:3000/gastocalorico/gastos/${user.id}`);
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              const response = await axios.delete(`http://192.168.0.107:3000/gastocalorico/delete/${id}`);
