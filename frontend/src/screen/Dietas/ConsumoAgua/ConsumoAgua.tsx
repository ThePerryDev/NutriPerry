...
const ConsumoAguaScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchConsumoAgua = async () => {
    try {
      const response = await axios.get(
        `http://192.168.0.107:3000/consumo-agua/${userId}`
      );
  ...
  const handleAddConsumo = async () => {
    ...
    try {
      await axios.post("http://192.168.0.107:3000/consumo-agua", {
        user: userId,
  ...
  const handleDeleteConsumo = async (date: string) => {
    ...
    try {
      await axios.delete(
        `http://192.168.0.107:3000/consumo-agua/${userId}/${formattedDate}`
      );
