...
const ConsumoAguaScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchConsumoAgua = async () => {
    try {
      const response = await axios.get(
        `http://10.68.55.162:3000/consumo-agua/${userId}`
      );
  ...
  const handleAddConsumo = async () => {
    ...
    try {
      await axios.post("http://10.68.55.162:3000/consumo-agua", {
        user: userId,
  ...
  const handleDeleteConsumo = async (date: string) => {
    ...
    try {
      await axios.delete(
        `http://10.68.55.162:3000/consumo-agua/${userId}/${formattedDate}`
      );
