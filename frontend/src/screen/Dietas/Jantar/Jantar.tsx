...
const CafedaManha: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchAlimentos = async () => {
    try {
      ...
      const responseAlimentos = await axios.get("http://10.68.55.203:3000/consumos/alimento", {
        params: {
          userId: user?.id,
          data: formattedDate,
          tipoRefeicao: "jantar"
      ...
      const responseTotais = await axios.get("http://10.68.55.203:3000/consumos/listarconsumo", {
        params: {
          userId: user?.id,
          data: formattedDate,
          tipoRefeicao: "jantar"
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`http://10.68.55.203:3000/consumos/delete/${id}`);
